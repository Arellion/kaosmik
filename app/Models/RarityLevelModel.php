...
class RarityLevelModel extends Model
{
    ...
    /**
     * Empêche la suppression du commun
     * @throws \Exception
     */
    protected function protectDefaultRarity(array $data) {
        $id = $data['id'][0] ?? null;
        if($id == 1) {
            throw new \Exception('Interdiction de modifier ou de supprimer la rareté par défaut (commun)');
        }
        return $data;
    }
protected function calculateCommonAppearanceRate(array $data)
{
    ...
    //on bloque toujour quand même toujours le 1
    if($id == 1) {
        return $data;
    }
    //on calcul de toutes les rareté autre que commun
