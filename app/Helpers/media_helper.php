...
if (!function_exists('upload_single_image')) {
    ...
    )
    {
        ...
        if (!in_array($mime_type, $accepted_mime_types)) {
            return ['status' => 'error', 'message' => 'Le type de fichier non accepté.'];
        }
        if ($file->getSizeByBinaryUnit(\CodeIgniter\Files\FileSizeUnit::KB) > $max_size) {
            return ['status' => 'error', 'message' => 'Le fichier est trop volumineux.'];
        }
        //2 - Definir le dossier de destination
        $year = date('y');
        $month = date('m');
        ...
        helper('text');
        $basename = $custom_name ? url_title($custom_name, '-', TRUE) : pathinfo($file->getClientName(), PATHINFO_FILENAME);
